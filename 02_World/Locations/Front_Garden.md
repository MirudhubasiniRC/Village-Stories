# Front Garden

## Purpose

The Front Garden is the welcoming face of the Family House.

It forms the transition between the public village and the family's private home.

Every visitor, neighbour, family member, and traveller first experiences the house through this garden.

It should immediately communicate warmth, care, and the family's close relationship with nature.

---

# Canon Visual Reference

Wide establishing (house + thinnai + landscape):

```text
11_Assets/Location_References/Family_House_Establishing_Canon_Design_Reference.png
```

Front garden planting density + center path:

```text
11_Assets/Location_References/Front_Garden_Canon_Design_Reference.png
```

**Approved look (CANON-2026-008 — 2026-07-18):** Working **homestead yard** with
**dense green kitchen garden** (not bare dirt only, not pretty cottage). Packed
earth path · **kolam** near thinnai · **Tulsi madam** · chickens · **one narrow
center path** gate → thinnai, flanked by lush greens: curry leaf · lemon ·
banana · chilli · tomato · marigold · hibiscus · bougainvillea · more trees.
House = **thatch + earthen walls**. Long shots also show cow/goat/chicken
shelters, well under big tree, water beyond rear, close neighbour roofs (see
`Family_House.md` § Long-shot establishing).
**NO compound / perimeter wall** — open village approach. Neighbours **close and
irregular** (not perfect grid lines); soft **village green path**; river + paddy
fields in the wider frame; jackfruit, banana, mango near house and shelters.

---

# Identity

Unlike a decorative garden, this is a living family space.

Every plant has a purpose.

Some provide flowers for prayer.

Some provide ingredients for cooking.

Some offer shade.

Others simply make the home beautiful.

The garden should feel mature, naturally grown over many years, and lovingly maintained.

---

# Location

The Front Garden is permanently located between the Main Village Path and the Front Thinnai.

It is the easternmost part of the family property.

Morning sunlight reaches this area before any other part of the house.

---

# Overall Layout

```text
                 Main Village Path
                        │
              Wooden Entrance Gate (low · open)
                        │
    ┌─────────────────────────────────────────────┐
    │  FULL LUSH GARDEN · dense green both sides   │
    │  mango · curry leaf · lemon · chilli · tomato│
    │  banana · hibiscus · marigold · jasmine      │
    │         │  CENTER PATH ONLY  │               │
    │         │  (packed earth)    │               │
    └─────────────────────────────────────────────┘
                        │
        ┌───────────────────────────────────┐
        │   WIDE SHALLOW Front Thinnai        │
        └───────────────────────────────────┘
```

**Layout lock:** The front yard is **entirely garden** — vegetables, fruit trees,
flowers, and herbs fill **both sides** of a **single narrow center path**. No bare
open lawn · no wide empty dirt plaza. Path worn smooth · edges soft with creeping grass
and fallen petals.

**Front Garden planting lock (approved 2026-07-17):**

- **Density:** Post-Monsoon **full green** — lush, juicy foliage everywhere; garden
  feels thick, mature, and lovingly tended.
- **Center path only** — one straight-ish packed-earth aisle gate → thinnai steps.
- **Curry leaf tree (karuveppilai maram)** — small/medium tree in garden (left or
  right of path); dense compound leaves · daily cooking harvest.
- **Lemon tree (nimbu)** — compact citrus with **green/yellow lemons** visible.
- **Large mango tree(s)** — ripe yellow-orange fruit hanging.
- **Kitchen beds in garden:** green **chilli**, **tomato** (red fruit on vine),
  brinjal/okra at edges — woven through flowers, not isolated strip.
- **Banana** clumps · **hibiscus** · **jasmine** · **marigold** · **bougainvillea**
  on house wall · optional **Tulsi** pedestal near path edge.
- Match `Front_Garden_Canon_Design_Reference.png` for density + path;
  `Family_House_Establishing_Canon_Design_Reference.png` for house + distant landscape.

Nothing should feel perfectly symmetrical.

The garden should look naturally developed over time.

---

# Entrance Gate

Maintain:

* Handmade wooden gate.
* Low wooden fence on both sides.
* Simple wooden latch.
* Weathered finish.
* Flowering creepers growing nearby.

The gate usually remains open during the day unless livestock need to be kept inside.

---

# Entrance Path

Maintain:

* **Single center path only** — narrow packed earth running gate → thinnai.
* Garden fills **both sides** completely; path is the only clear walkway.
* Slightly worn by years of walking · grass and flowers creeping at edges.
* Small natural stones · fallen flowers · leaves after windy days.

The path should gently guide visitors toward the Front Thinnai through lush greenery.

---

# Front Kitchen Vegetable Beds

Vegetables grow **inside the full front garden** — on both sides of the center path —
not as a separate bare strip.

Maintain:

* **Green chilli plants** — bushy rows among flowers.
* **Tomato vines** — stakes/trellis; **ripe red tomatoes** visible.
* Brinjal · okra · cluster beans at bed edges — never bare soil patches.
* Rich brown soil · small earthen borders · garden feels **overflowing green**.
* Hand-watered practical kitchen garden woven with fruit trees and flowers.

**Fruit trees in front garden (required):**

* **Curry leaf tree** — medium height beside path; dense karuveppilai foliage.
* **Lemon tree** — compact; green/yellow lemons on branches.
* **Mango tree(s)** — large; ripe hanging fruit.

Purpose:

* Daily harvest — Mother plucks curry leaves, lemon, chilli, tomato on her way in.
* Complements the larger **Vegetable Garden** behind the house.

Seasonal note: peak abundance Post-Monsoon / early harvest — match
`Agricultural_Calendar.md` when story timing matters.

---

# Flower Garden

Flowers are an essential part of the home's identity.

Maintain a mixture of:

* Jasmine.
* Hibiscus.
* Kanakambaram (Firecracker Flower).
* Marigold.
* Rose (one or two traditional bushes).
* Butterfly-attracting wildflowers.

These flowers are used for:

* Daily prayer.
* Hair decoration.
* Festival decoration.
* Natural beauty.

Flowers should bloom differently throughout the seasons.

---

# Sacred Tulsi

One raised Tulsi platform permanently exists within the Front Garden.

Maintain:

* Lime-plastered square pedestal.
* Healthy Tulsi plant.
* Small oil lamp space.
* Fresh water offered daily.
* Occasional flower offerings.

The Tulsi is part of the family's daily routine.

---

# Trees

The Front Garden is **tree-rich** — not flower-only beds.

Required / primary:

* **Large mango tree(s)** — ripe yellow-orange mangoes when in season.
* **Curry leaf tree (karuveppilai maram)** — cooking harvest; dense small compound leaves.
* **Lemon tree (nimbu)** — compact citrus; fruit visible on branches.

Also maintain:

* **Banana** clumps with broad fronds.
* Coconut/palm in mid-ground or framing (ultra-wide shots).
* Guava or amla optional at garden edge.

Trees should:

* Fill the garden with shade dapple and deep green canopy layers.
* Attract birds.
* Never block the wide shallow thinnai façade entirely.

---

# Butterflies & Pollinators

The Front Garden naturally attracts:

* Butterflies.
* Honey bees.
* Dragonflies after rain.
* Small birds.

Their presence should subtly enrich the environment.

---

# Daily Life

Morning

* Mother collects flowers.
* Tulsi worship.
* Sweeping.
* Fresh kolam at the entrance.
* Children leaving for school.

Afternoon

* Dog resting in the shade.
* Butterflies visiting flowers.
* Neighbours stopping briefly.
* Light gardening.

Evening

* Family returning home.
* Soft conversations near the gate.
* Children playing before dinner.
* Oil lamp near the Tulsi.

---

# Seasonal Behaviour

## Summer

* Bright flowering plants.
* Dry pathway.
* Deep morning sunlight.
* Warm earthy colours.

---

## Monsoon

* Rich green foliage.
* Rain droplets on leaves.
* Wet earth.
* Fresh flower growth.
* Butterflies after rainfall.

---

## Post-Monsoon / Harvest

* Most colourful season.
* Full flowering plants.
* Healthy vegetation.
* Fresh fragrance.

---

# Environmental Behaviour

Always include naturally when appropriate:

* Bees moving between flowers.
* Butterflies.
* Fallen petals.
* Gentle breeze moving leaves.
* Morning dew.
* Light drifting flower fragrance.
* Birds visiting the tree.

The garden should never appear static.

---

# Audio Identity

Common sounds include:

* Bees.
* Butterflies fluttering softly.
* Birds singing.
* Leaves moving in the breeze.
* Footsteps along the entrance path.
* Children entering and leaving.
* Dog resting nearby.

The atmosphere should feel peaceful and welcoming.

---

# Story Opportunities

The Front Garden naturally supports:

* Welcoming visitors.
* Flower gathering.
* Festival decoration.
* Children returning from school.
* Quiet conversations.
* Receiving deliveries.
* Morning routines.
* Family farewells.
* Village greetings.

It often serves as the transition between the public village and the private home.

---

# Camera Notes

Useful compositions include:

* Looking from the Village Path toward the house.
* Looking outward from the Front Thinnai.
* Flowers framing the foreground.
* Butterflies passing through the frame.
* Morning sunlight illuminating the entrance.
* Low-angle shots through flower beds.
* The house partially hidden behind plants for depth.

The garden should make the house feel inviting before viewers even enter it.

---

# Continuity Rules

Always maintain:

* Wooden entrance gate.
* **Center path only** through full lush garden.
* **Curry leaf tree + lemon tree + mango tree** in front garden.
* Front kitchen vegetables (chilli · tomato minimum).
* Flower beds woven through greenery.
* Tulsi platform.
* Open relationship with the Front Thinnai.
* Traditional planting style · **dense Post-Monsoon green**.

Never:

* Add ornamental concrete landscaping.
* Introduce modern garden decorations.
* Replace flowers with lawns.
* Install paving stones.
* Remove the Tulsi platform.
* Introduce exotic ornamental plants inconsistent with a 1990s South Indian village.

---

# Canon Authority

This document is the permanent reference for the Front Garden.

Every future storyboard, image prompt, animation, and story involving the front of the Family House must preserve this layout and atmosphere unless a permanent canonical change is intentionally introduced.

---

# Guiding Principle

The Front Garden is the home's first smile.

Before a character speaks, before a meal is cooked, and before a story begins, the garden quietly welcomes everyone with flowers, birdsong, and the feeling that this is a home cared for with love.
