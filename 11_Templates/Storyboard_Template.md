# Storyboard / Shot List Template

| Field | Value |
| --- | --- |
| Episode ID | `ep_` |
| Title | |
| Format / aspect | |
| Status | draft / in_review / approved |
| Revision | r01 |
| Linked script | |

---

## Scene-state ledger (fill before generation)

| Shot | Location | Cast present | Wardrobe / props | Light direction | Weather | Geography notes | Open risks |
| --- | --- | --- | --- | --- | --- | --- | --- |
| sh_001 | | | | | | | |

---

## Shot list

### sh_001

| Element | Specification |
| --- | --- |
| Scene | |
| Duration target | |
| Story purpose | Hook / Orient / Process / Turn / Resolve / Rest |
| Emotional objective | |
| Subject | |
| Action (frozen for still) | |
| Shot type | |
| Camera height | |
| Camera angle | |
| Lens feeling | |
| Composition | |
| Depth of field | |
| Characters visible | |
| Location lock source | |
| Character lock sources | |
| Object lock sources | |
| Reference assets (M-61 roles) | |
| Motion intent (later Flow) | One principal action: |
| Audio intent | |
| Dialogue / narration | None / … |
| Negatives (clip-specific) | |
| Status | OPEN / HOLD FOR BOARD / approved still pending |

---

### sh_002

*(Duplicate shot block.)*

---

## Board QA

- [ ] Every shot has purpose + emotional objective
- [ ] Screen direction consistent along route
- [ ] Crop-safe for declared aspect
- [ ] One principal action per clip planned
- [ ] Scene-state ledger complete
- [ ] Ready for Image Prompt Template (phased)

Authority: `08_Production/05_Storyboard_Workflow.md` · `09_AI_Production_System/19_Camera_Direction_Bible.md` · `20_Camera_Movement_Bible.md`
